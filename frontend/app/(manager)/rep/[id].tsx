// Manager drill-down: individual rep profile with KPIs + today's session + leads
...
import { fmtDuration, fmtDistanceKm } from "../../../src/lib/useWorkStatus";
...
export default function ManagerRepProfile() {
  ...
  // Build pin list for the mini-map
  ...
  const lastLoc = data.track && data.track.length > 0 ? data.track[data.track.length - 1] : null;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="rep-profile-screen">
      <View style={styles.header}>
        <TouchableOpacity style={styles.back} onPress={() => router.back()} testID="rep-back">
          <Feather name="arrow-left" size={20} color={colors.textPrimary} />
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={styles.heroCard}>
          <View style={styles.avatar}>
            ...
            <View style={[styles.statusDot, { backgroundColor: data.kpi.is_working ? colors.secondary : "#94A3B8" }]} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.heroName}>{data.user.name || "—"}</Text>
            <Text style={styles.heroEmail}>{data.user.email}</Text>
            <View style={styles.badgesRow}>
              <View style={[styles.badge, { backgroundColor: data.kpi.is_working ? `${colors.secondary}20` : "#F1F5F9" }]}>
                <View style={[styles.badgeDot, { backgroundColor: data.kpi.is_working ? colors.secondary : "#94A3B8" }]} />
                <Text style={[styles.badgeText, { color: data.kpi.is_working ? colors.secondary : colors.textSecondary }]}>
          ...
          <View style={styles.sessionCard}>
            ...
            <View style={styles.sessionRow}>
              <View style={styles.sessionItem}>
                ...
                <Text style={styles.sessionValue}>{fmtDuration(data.kpi.session_seconds)}</Text>
              ...
              <View style={styles.sessionItem}>
                ...
                <Text style={styles.sessionValue}>{fmtDistanceKm(data.kpi.session_distance_m)}</Text>
        ...
        <View style={styles.commissionCard}>
          ...
          <View style={styles.commissionRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.commissionValue, { color: colors.secondary }]}>{fmtPln(data.kpi.commission_payable)}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.commissionLabel}>Zamrożone (14d)</Text>
              <Text style={[styles.commissionValue, { color: colors.info }]}>{fmtPln(data.kpi.commission_frozen)}</Text>
            </View>
          </View>
        </View>
        ...
        {(pins.length > 0 || repPin.length > 0) && (
          <View style={styles.mapCard}>
            <Text style={styles.sectionLabel}>Mapa (leady + trasa dzisiejsza)</Text>
            <LeadMap
              pins={pins}
              reps={repPin}
          ...
          <View style={{ gap: 8 }}>
            {Object.entries(data.status_breakdown || {}).map(([s, n]) => (
              <View key={s} style={styles.statusRow}>
                <View style={[styles.statusDotSm, { backgroundColor: statusColor[s] || "#94A3B8" }]} />
          ...
          {data.leads.length === 0 ? (
            ...
            data.leads.map((l) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <View style={[styles.statusDotSm, { backgroundColor: statusColor[l.status] || "#94A3B8" }]} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.leadName}>{l.client_name || "—"}</Text>
                  <Text style={styles.leadSub} numberOfLines={1}>
                    {l.address || "brak adresu"} · {statusLabel[l.status] || l.status}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  heroCard: { flexDirection: "row", gap: 12, alignItems: "center", backgroundColor: colors.paper, borderRadius: radius.lg, padding: spacing.md, borderWidth: 1, borderColor: colors.border },
  avatar: { width: 60, height: 60, borderRadius: 30, backgroundColor: colors.primary, alignItems: "center", justifyContent: "center", position: "relative" },
  avatarText: { color: "#fff", fontWeight: "900", fontSize: 18 },
  statusDot: { position: "absolute", bottom: 2, right: 2, width: 14, height: 14, borderRadius: 7, borderWidth: 2, borderColor: colors.paper },
  ...
  sessionCard: { backgroundColor: colors.paper, borderRadius: radius.lg, padding: spacing.md, marginTop: 12, borderWidth: 1, borderColor: colors.border },
  sectionLabel: { fontSize: 10, color: colors.textSecondary, fontWeight: "800", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 },
  sessionRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-around" },
  sessionItem: { alignItems: "center", gap: 4, flex: 1 },
  sessionValue: { fontSize: 18, fontWeight: "900", color: colors.textPrimary, fontVariant: ["tabular-nums"] },
  sessionLabel: { fontSize: 10, color: colors.textSecondary, fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5 },
  sessionSep: { width: 1, height: 36, backgroundColor: colors.border },
  kpiGrid: { flexDirection: "row", gap: 8, flexWrap: "wrap", marginTop: 12 },
  kpi: { flexBasis: "48%", flexGrow: 1, backgroundColor: colors.paper, padding: 14, borderRadius: radius.md, borderLeftWidth: 4, borderWidth: 1, borderColor: colors.border },
  kpiLabel: { fontSize: 11, color: colors.textSecondary, fontWeight: "800", textTransform: "uppercase", letterSpacing: 1 },
  kpiValue: { fontSize: 24, fontWeight: "900", color: colors.textPrimary, marginTop: 4, fontVariant: ["tabular-nums"] },
  commissionCard: { backgroundColor: colors.paper, borderRadius: radius.lg, padding: spacing.md, marginTop: 12, borderWidth: 1, borderColor: colors.border },
  commissionRow: { flexDirection: "row", gap: 12 },
  commissionLabel: { fontSize: 11, color: colors.textSecondary, fontWeight: "700" },
  commissionValue: { fontSize: 18, fontWeight: "900", marginTop: 4, fontVariant: ["tabular-nums"] },
  mapCard: { backgroundColor: colors.paper, borderRadius: radius.lg, padding: spacing.md, marginTop: 12, borderWidth: 1, borderColor: colors.border },
  statusCard: { backgroundColor: colors.paper, borderRadius: radius.lg, padding: spacing.md, marginTop: 12, borderWidth: 1, borderColor: colors.border },
  statusRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  statusDotSm: { width: 10, height: 10, borderRadius: 5 },
  statusName: { flex: 1, fontSize: 13, color: colors.textPrimary, fontWeight: "700" },
